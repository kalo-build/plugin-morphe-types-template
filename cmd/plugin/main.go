...
import (
	...
	"github.com/kalo-build/plugin-morphe-types-template/pkg/compile"
)
...
func main() {
	...
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: plugin-morphe-_FORMAT_-types <config>")
		fmt.Fprintln(os.Stderr, "  config: JSON string with inputPath, outputPath, and optional config parameters")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Example:")
		fmt.Fprintln(os.Stderr, `  plugin-morphe-_FORMAT_-types '{"inputPath":"./morphe","outputPath":"./output","verbose":true}'`)
	...
	logInfo(compileConfig.Verbose, "Output _FORMAT_ types to: '%s'", compileConfig.OutputPath)
	...
	if err := compile.MorpheTo_FORMAT_(morpheConfig); err != nil {
